...
public class ProvinceUnionAuditListener implements TaskListener {
    ...
    public void notify(DelegateTask delegateTask) {
        try {
            ...
            String provinceOpinion = (String) vars.get("province_opinion");
            // 省总工会审批负责人（表单字段名：province_leader_name）
            String provinceLeaderName = (String) vars.get("province_leader_name");
            // 省总工会审批经办人（表单字段名：province_handler_name）
            String provinceHandlerName = (String) vars.get("province_handler_name");
            ...
            if (data == null) {
                System.out.println("省总工会审批：未查询到流程ID[" + processId + "]的业务数据");
                return;
            }
            System.out.println("省总工会审批：查询到的业务表数据：id=" + data.getId());

            // ========== 6. 写入省总工会专属业务字段 ==========
            data.setProvinceOpinion(provinceOpinion); // 省总工会审批意见
            data.setProvinceLeaderName(provinceLeaderName); // 省总工会负责人
            data.setProvinceHandlerName(provinceHandlerName); // 省总工会经办人
            data.setProvinceApproveTime(approveTime); // 审批时间（也可从vars取）

            // ========== 7. 更新数据库 ==========
            wfTdfSqMapper.updateById(data);
            System.out.println("===== 省总工会审批数据同步完成 =====");
