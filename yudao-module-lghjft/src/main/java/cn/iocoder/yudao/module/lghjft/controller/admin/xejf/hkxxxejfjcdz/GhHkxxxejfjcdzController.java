...
public class GhHkxxxejfjcdzController {
    ...
    @Operation(summary = "导出小额缴费基层到账 Excel")
    @PreAuthorize("@ss.hasPermission('lghjft:xejf-hkxxxejfjcdz:export')")
    @ApiAccessLog(operateType = EXPORT)
    public void exportExcel(@Valid GhHkxxxejfjcdzPageReqVO pageReqVO,
                            HttpServletResponse response) throws IOException {
        pageReqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        List<GhHkxxxejfjcdzDO> list = ghHkxxxejfjcdzService.getGhHkxxxejfjcdzPage(pageReqVO).getList();
        ExcelUtils.write(response, "小额缴费基层到账.xls", "数据", GhHkxxxejfjcdzResVO.class,
                BeanUtils.toBean(list, GhHkxxxejfjcdzResVO.class));
    }
}
