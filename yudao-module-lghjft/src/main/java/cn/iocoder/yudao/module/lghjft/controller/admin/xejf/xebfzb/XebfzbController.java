...
public class XebfzbController {
    ...
    @Operation(summary = "导出小额拨付占比 Excel")
    @PreAuthorize("@ss.hasPermission('lghjft:xejf-xebfzb:export')")
    @ApiAccessLog(operateType = EXPORT)
    public void exportExcel(@Valid XebfzbPageReqVO pageReqVO,
                            HttpServletResponse response) throws IOException {
        pageReqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        List<XebfzbDO> list = xebfzbService.getXebfzbPage(pageReqVO).getList();
        ExcelUtils.write(response, "小额拨付占比.xls", "数据", XebfzbResVO.class,
                BeanUtils.toBean(list, XebfzbResVO.class));
    }
}
